  ...
  return (
    <div className="page nabb-league">
      <div className="page-header">
        <h1 className="gradient-text">âš¾ NABB</h1>
...
const OverviewTab = () => {
  ...
  useEffect(() => {
    db.getTeams('nabb').then(setTeams);
    db.getPlayers('nabb').then(setPlayers);
    db.getBsGames('nabb').then(setBsGames);
  }, []);
...
const RostersTab = ({ onSelectPlayer }) => {
  ...
  useEffect(() => {
    Promise.all([db.getTeams('nabb'), db.getPlayers('nabb')])
...
const LeagueLeadersTab = ({ onSelectPlayer }) => {
  ...
  useEffect(() => {
    db.getPlayers('nabb').then(setPlayers);
    db.getBoxScores('nabb').then(setBoxScores);
  }, []);
...
const GameFeedTab = () => {
  ...
  useEffect(() => { db.getFeed('nabb').then(setFeed); }, []);
...
const BoxScoresTab = () => {
  const [bsGames, setBsGames] = useState([]);
  useEffect(() => { db.getBsGames('nabb').then(setBsGames); }, []);
  const [boxScores, setBoxScores] = useState([]);
  useEffect(() => { db.getBoxScores('nabb').then(setBoxScores); }, []);
  const [players, setPlayers] = useState([]);
  useEffect(() => { db.getPlayers('nabb').then(setPlayers); }, []);
  const [teams, setTeams] = useState([]);
...
const HallOfFameTab = () => {
  ...
  useEffect(() => { db.getHof('nabb').then(setHof); }, []);
