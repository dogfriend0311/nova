    ...
    <div className="tab-content">
      ...
      <div className="neon-card p-3" style={{ marginTop:'20px' }}>
        <div className="edit-form">
          {['bio','top_banner_url','left_banner_url','right_banner_url','spotify_url','twitter_url','twitch_url','youtube_url','instagram_url'].map(f => (
...
const LeaguePlayersTab = ({ prefix }) => {
  ...
  const del = async (id) => {
    await db.deletePlayer(prefix, id);
    setPlayers(prev => prev.filter(p => p.id !== id));
  };

  const startEdit = (p) => {
    setEditing(p.id);
    setForm({ ...emptyPlayer, ...p });
          ...
          <div className="form-field">
            ...
            {avatarPreview && (
              <div style={{ marginTop:'12px' }}>
                <div
                  style={{ width:'120px', height:'120px', overflow:'hidden', border:'2px solid rgba(0,255,255,0.3)', borderRadius:'8px', cursor:dragging?'grabbing':'grab', position:'relative', background:'#0a0a23', userSelect:'none' }}
                ...
                <div style={{ marginTop:'10px', display:'flex', alignItems:'center', gap:'10px' }}>
                  ...
                  <input type="range" min="0.5" max="3" step="0.05" value={zoom}
                    onChange={e => { setZoom(parseFloat(e.target.value)); setTimeout(() => { const d = bakeAvatar(); if (d) setForm(f => ({ ...f, avatar_data: d })); }, 50); }}
                    style={{ flex:1 }} />
            ...
            <div key={p.id} className="neon-card p-3" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'10px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                {p.avatar_data
                  ...
                  : <div style={{ width:'44px', height:'44px', borderRadius:'6px', background:'rgba(0,255,255,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.4rem' }}>🎮</div>
                }
                <div>
        ...
        <div className="edit-form">
          ...
          <div className="form-field">
            ...
            <div style={{ display:'flex', gap:'10px', marginBottom:'10px' }}>
              {['url','upload'].map(m => (
                <button key={m} type="button" onClick={() => setUploadMode(m)} style={{ padding:'6px 14px', background:uploadMode===m?'rgba(0,255,255,0.2)':'rgba(0,255,255,0.05)', border:'1px solid rgba(0,255,255,0.3)', color:'var(--color-cyan)', borderRadius:'4px', cursor:'pointer', fontSize:'0.85rem' }}>
                  {m === 'url' ? 'URL' : 'Upload'}
                </button>
              ))}
            </div>
            ...
            <div key={team.id} className="neon-card p-3">
              <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'15px' }}>
                {team.logo_url
                  ...
                  : <div style={{ width:'48px', height:'48px', background:team.team_color, borderRadius:'6px' }} />}
...
const LeagueRostersTab = ({ prefix }) => {
  ...
  useEffect(() => {
    db.getTeams(prefix).then(setTeams);
  ...
  const clearRoster = async () => {
    ...
    for (const p of teamPlayers) await db.savePlayer(prefix, { ...p, team: '' });
  ...
  return (
    <div className="tab-content">
      <div style={{ display:'flex', gap:'10px', marginBottom:'20px', flexWrap:'wrap' }}>
        ...
        {teamPlayers.length > 0 && <button className="neon-button" style={{ borderColor:'#ff3333', color:'#ff3333' }} onClick={clearRoster}>🗑️ Clear Roster</button>}
      </div>
...
const LeagueGamesTab = ({ prefix }) => {
  ...
  useEffect(() => {
    db.getGames(prefix).then(d => { setGames(d); setLoading(false); });
    db.getTeams(prefix).then(setTeams);
  }, [prefix]);

  const addGame = async () => {
    if (!newGame.home_team || !newGame.away_team || newGame.home_team === newGame.away_team) return;
    const ht = teams.find(t => t.team_name === newGame.home_team);
    const at = teams.find(t => t.team_name === newGame.away_team);
  ...
  const updateGame = async () => {
    ...
    setGames(prev => prev.map(g => g.id === editing ? saved : g));
    ...
    <div className="tab-content">
      ...
      <div className="neon-card p-3" style={{ marginTop:'20px', marginBottom:'30px' }}>
        ...
        <div className="edit-form">
          ...
          <div className="form-field"><label>Date &amp; Time</label><input type="datetime-local" value={newGame.game_date} onChange={e=>setNewGame({...newGame,game_date:e.target.value})} style={SI} /></div>
...
const LeagueBoxScoresTab = ({ prefix }) => {
  ...
  useEffect(() => {
    db.getBsGames(prefix).then(setBsGames);
    db.getBoxScores(prefix).then(setBoxScores);
    db.getPlayers(prefix).then(setPlayers);
    db.getTeams(prefix).then(setTeams);
  }, [prefix]);
  ...
  const createGame = async () => {
    ...
    setBsGames(prev => [saved, ...prev]);
    setNewGame({ game_name:'', home_team:'', away_team:'', home_score:0, away_score:0, game_date:'' });
    setShowCreate(false);
  };

  const deleteGame = async (id) => {
    await db.deleteBsGame(prefix, id);
    setBsGames(prev => prev.filter(g => g.id !== id));
    setBoxScores(prev => prev.filter(b => b.game_id !== id));
  };
  ...
  const saveScore = async () => {
    ...
    setBoxScores(prev => prev.map(b => b.id === editingScore.id ? saved : b));
                  ...
                  return (
                    <tr key={score.id} style={{ borderBottom:'1px solid rgba(0,255,255,0.05)' }}>
                      ...
                      <td style={{ padding:'8px', textAlign:'center' }}>
                        <button onClick={() => { setEditingScore(score); setEditForm({...score}); }} style={{ background:'none', border:'none', color:'var(--color-cyan)', cursor:'pointer' }}>✏️</button>
                      </td>
                    </tr>
                  );
                })}
...
const LeagueGameFeedTab = ({ prefix }) => {
  ...
  useEffect(() => {
    db.getGames(prefix).then(setGames);
    db.getPlayers(prefix).then(setPlayers);
    db.getFeed(prefix).then(setFeed);
  }, [prefix]);
  ...
  if (!selectedGame) return (
    <div className="tab-content">
      ...
      <div style={{ marginTop:'20px' }}>
        {liveGames.length === 0
          ? <div className="neon-card p-3"><p style={{ color:'rgba(192,208,255,0.5)', textAlign:'center' }}>No live/final games. Set a game status to Live in the Games tab.</p></div>
          : liveGames.map(game => (
                ...
                {editingEvent === event.id ? (
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                    ...
                    <div style={{ display:'flex', gap:'6px' }}>
                      <button onClick={()=>{setEditingEvent(event.id);setEditNote(event.event_type);}} style={{ background:'none', border:'none', color:'var(--color-cyan)', cursor:'pointer' }}>✏️</button>
                      <button onClick={()=>deleteEvent(event.id)} style={{ background:'none', border:'none', color:'#ff3333', cursor:'pointer' }}>🗑️</button>
                    </div>
                  </div>
                )}
              </div>
...
const LeagueHofTab = ({ prefix }) => {
  ...
  const remove = async (id) => {
    await db.deleteHof(prefix, id);
    setHofMembers(prev => prev.filter(m => m.id !== id));
  };
      ...
      <div className="neon-card p-3" style={{ marginTop:'20px', marginBottom:'30px' }}>
        ...
        <div className="edit-form">
          ...
          <div className="form-field">
            ...
            <input type="number" value={form.year} onChange={e=>setForm({...form,year:+e.target.value})} style={SI} />
          </div>
...
const OwnerDashboard = ({ onExit }) => {
  ...
  const isNABB = role === 'nabb_helper';
  const isRBML = role === 'rbml_helper';
  const [activeTab, setActiveTab] = useState(
    isOwnerLevel ? 'member-pages' : isNABB ? 'nabb-players' : 'rbml-players'
  );
  ...
  return (
    <div className="owner-dashboard">
      ...
      <div className="dashboard-sections">
        ...
        {(isOwnerLevel || isNABB) && (
          <div className="dashboard-section">
            <div className="section-label">⚾ NABB</div>
            <div className="dashboard-tabs">
              <Btn id="nabb-players"   label="🎮 Players" />
              <Btn id="nabb-teams"     label="🏟️ Teams" />
              <Btn id="nabb-rosters"   label="👥 Rosters" />
              <Btn id="nabb-games"     label="📅 Games" />
              <Btn id="nabb-boxscores" label="📈 Box Scores" />
              <Btn id="nabb-feed"      label="📰 Feed" />
              <Btn id="nabb-hof"       label="🏆 HoF" />
            </div>
          </div>
        )}
        {(isOwnerLevel || isRBML) && (
          <div className="dashboard-section">
            <div className="section-label">⚾ RBML</div>
            <div className="dashboard-tabs">
              <Btn id="rbml-players"   label="🎮 Players" />
              <Btn id="rbml-teams"     label="🏟️ Teams" />
              <Btn id="rbml-rosters"   label="👥 Rosters" />
              <Btn id="rbml-games"     label="📅 Games" />
              <Btn id="rbml-boxscores" label="📈 Box Scores" />
              <Btn id="rbml-feed"      label="📰 Feed" />
              <Btn id="rbml-hof"       label="🏆 HoF" />
            </div>
          </div>
        )}
      </div>
