 ...
 *
 * Persistence: feed data is cached in localStorage so completed games
 * remain viewable after the API stops returning live data.
 */
...
// ── Cache helpers ─────────────────────────────────────────────
const mlbCacheKey  = (pk)      => `nova_pbp_mlb_${pk}`;
const espnCacheKey = (eventId) => `nova_pbp_espn_${eventId}`;
...
// ══════════════════════════════════════════════════════════════
//  BASEBALL PBP — MLB Stats API  (with localStorage cache)
// ══════════════════════════════════════════════════════════════
const BaseballPBP = ({ gamePk, game }) => {
  ...
  // Resolve gamePk (ESPN IDs are 9 digits, MLB PKs are 6-7)
  useEffect(() => {
  ...
  const fetchFeed = useCallback(async () => {
    ...
    // For completed games, serve from cache if available (avoid unnecessary API calls)
    if (isFinal) {
      const cached = readCache(mlbCacheKey(resolvedPk));
      if (cached) { setFeed(cached); setFromCache(true); return; }
    }
    ...
    try {
      ...
      writeCache(mlbCacheKey(resolvedPk), d);
      // Stop polling if game just ended
      if (d.gameData?.status?.abstractGameState === 'Final') {
        clearInterval(intervalRef.current);
      }
    } catch (e) {
      // Fallback to cache on any fetch error
      const cached = readCache(mlbCacheKey(resolvedPk));
      if (cached) {
        setFeed(cached);
        setFromCache(true);
  ...
  useEffect(() => {
    if (!resolvedPk) return;
    fetchFeed();
    // No polling needed for completed games
    if (!isFinal) {
      intervalRef.current = setInterval(fetchFeed, 15000);
      return () => clearInterval(intervalRef.current);
    }
  }, [fetchFeed, resolvedPk, isFinal]);
...
// ══════════════════════════════════════════════════════════════
//  FOOTBALL PBP — ESPN API  (with localStorage cache)
// ══════════════════════════════════════════════════════════════
const FootballPBP = ({ eventId, sport, game }) => {
  ...
  const fetchData = useCallback(async () => {
    if (isFinal) {
      const cached = readCache(espnCacheKey(eventId));
    ...
    try {
      ...
      setData(d);
      setFromCache(false);
      writeCache(espnCacheKey(eventId), d);
    } catch(e) {
      const cached = readCache(espnCacheKey(eventId));
      if (cached) { setData(cached); setFromCache(true); }
      else setError(e.message);
    }
  }, [eventId, path, isFinal]);
      ...
      {curDrive && (
        <div className="pbp-current-drive">
          ...
          <div className="pbp-drive-summary">
            {curDrive.yards !== undefined && <span>{curDrive.yards} yds</span>}
...
// ══════════════════════════════════════════════════════════════
//  BASKETBALL PBP — ESPN API  (with localStorage cache)
// ══════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════
//  HOCKEY PBP — ESPN API  (with localStorage cache)
// ══════════════════════════════════════════════════════════════
