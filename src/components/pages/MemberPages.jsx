...
const MemberProfileView = ({ member, onBack }) => {
  ...
  const presence_         = localStorage.getItem(`nova_presence_${member.username}`) || 'online';
  const presenceDotColor  = presence_ === 'online' ? '#43b581' : presence_ === 'idle' ? '#f04747' : '#747f8d';
  const presenceLabel     = presence_ === 'online' ? 'Online' : presence_ === 'idle' ? 'Do Not Disturb' : 'Invisible';
